function [price_6,CI_6]=Pricing_BasketOption(r,F0_2026,K,T,drift_2026,k_IG_2026, sigma_2026, theta_2026, Y_2026,...
...
%   CI_6       - Confidence interval for the estimated price.
    ...
    XT_2026=drift_2026*T+Y_2026*(theta_2026*dS_2026+sigma_2026*sqrt(dS_2026).*Z_2026);
    FT_2026=F0_2026*exp(XT_2026);
    ...
    XT_2028=drift_2028*T+Y_2028*(theta_2028*dS_2028+sigma_2028*sqrt(dS_2028).*Z_2028);
    FT_2028=F0_2028*exp(XT_2028);
    
    % Martingality check swap 2028
    disp('Martingality check swap 2028: must be one')
    fprintf('E[exp(Xt)]: %d\n', mean(exp(XT_2028)))
    
    %% Option Pricing
    discpayoff=max(max(FT_2026,FT_2028)-K,0)*exp(-r*T);
    [price_6,~,CI_6]=normfit(discpayoff);
    ...
    %% check call option
    % 2026
    payoff_call_2026 = max(FT_2026-K,0)*exp(-r*T);
    ...
    % 2028
    payoff_call_2028 = max(FT_2028-K,0)*exp(-r*T);
