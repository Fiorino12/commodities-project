function [price_AVS,CI_AVS]=LookbackPrice_AVS(r,F0,K,dt,T,drift,k_IG, sigma, theta, Y,Nsim,M)
...
%   CI_AVS    : Confidence interval for the option price
    ...
    for j = 1:M
        % Anthitetic variable IG:
        [dS1, dS2] = fastInverseGaussian(dt, dt^2 / k_IG, Nsim);
        X_new_1 = X_old_1 + drift * dt + ...
            Y * (theta * dS1 + sigma * sqrt(dS1) .* Z(:, j));
        X_max_1 = max(X_new_1, X_max_1);
        X_old_1 = X_new_1;

        X_new_2 = X_old_2 + drift * dt + ...
            Y * (theta * dS2 + sigma * sqrt(dS2) .* Z(:, j));
    ...
    % Pricing   
    Payoff_disc_AVS_1 = max(F0 * exp(X_max_1) - K, 0) * exp(-r * T);
    Payoff_disc_AVS_2 = max(F0 * exp(X_max_2) - K, 0) * exp(-r * T);
    [price_AVS, ~, CI_AVS] = normfit((Payoff_disc_AVS_1 + Payoff_disc_AVS_2) / 2);

    % Create table for Price and Confidence Interval (CI)
    results_table = table(price_AVS, CI_AVS(1), CI_AVS(2),CI_AVS(2)-CI_AVS(1), 'VariableNames', {'Price_AVS', 'CI_Lower', 'CI_Upper', 'CI_length'});
